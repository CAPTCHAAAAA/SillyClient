...
正式入口从首页开始；旧预览参数 `section=showcase` 不得触发自动翻页或延迟跳转。
...
- 移动入口复用桌面设备 GLB、灯光与屏幕材质；WebGL 初始化失败时保留静态产品帧回退。第三页只允许懒加载的 B 站视频 iframe。

## 5. 移动入口

移动页保留桌面页的主题、背景和三页内容结构。首屏与第二页由
`mobile/scripts/webgl-stage.js` 在同一透明画布内实时渲染，复用桌面页的设备模型、
屏幕材质、灯光和三组站位。三个设备状态仍由 `mobile/scripts/frame-stage.js`
维护，并驱动 WebGL 转场：
...
表中的静态图是 WebGL 不可用时的回退内容。每张图为 `3840 × 2160`、带透明通道
的 WebP，三张图总传输大小不得超过 256 KiB。移动入口直接进入首页，不设置额外
开屏确认层。
...
| 移动 | WebGL 舞台正常，三种状态完整显示；WebGL 失败时产品帧回退可用 |
