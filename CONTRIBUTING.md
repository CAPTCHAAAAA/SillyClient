...
共享 React 控制台的唯一源码是 Android 仓库中的 `App/web/capacitor-ui/`。Windows 的 `frontend-dist/` 和主仓库的 `docs/app/` 都是它的构建副本，不在副本上直接开发。

Pages 副本从主仓库同步：

```bash
node scripts/sync-pages-app.mjs ../SillyClient_Android/App/web/capacitor-ui/dist
```
